...
payments.get('/', async (c) => {
  ...
  const filter: { tenantId: IUser['tenantId']; invoiceId?: string } = { tenantId: user.tenantId }
...
payments.post('/', requireRole('owner', 'admin'), async (c) => {
  ...
  if (numericAmount <= 0) return c.json({ error: 'Amount must be greater than 0' }, 400)
  if (numericAmount > invoice.balanceDue) return c.json({ error: 'Amount exceeds invoice balance' }, 400)
