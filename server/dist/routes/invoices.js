...
invoices.post('/', requireRole('owner', 'admin'), async (c) => {
    ...
    const { customerId, invoiceNumber, invoiceDate, dueDate, notes, terms, lineItems = [], currency = 'USD', } = body;
...
invoices.patch('/:id', requireRole('owner', 'admin'), async (c) => {
    ...
    const invoice = await Invoice.findOneAndUpdate({ _id: id, tenantId: user.tenantId }, updates, { new: true });
    if (!invoice)
        return c.json({ error: 'Invoice not found' }, 404);
    await logAudit({
        tenantId: user.tenantId,
...
invoices.post('/:id/send', requireRole('owner', 'admin'), async (c) => {
    const user = c.get('user');
    const id = c.req.param('id');
    const invoice = await Invoice.findOneAndUpdate({ _id: id, tenantId: user.tenantId }, { status: InvoiceStatus.SENT }, { new: true });
    if (!invoice)
        return c.json({ error: 'Invoice not found' }, 404);
    await logAudit({
        tenantId: user.tenantId,
...
invoices.post('/:id/cancel', requireRole('owner', 'admin'), async (c) => {
    const user = c.get('user');
    const id = c.req.param('id');
    const invoice = await Invoice.findOneAndUpdate({ _id: id, tenantId: user.tenantId }, { status: InvoiceStatus.CANCELLED }, { new: true });
    if (!invoice)
        return c.json({ error: 'Invoice not found' }, 404);
    await logAudit({
        tenantId: user.tenantId,
